...
async function assignLeads() {
  try {
    ...
    console.log(`\n📦 Found ${activeCustomers.length} active customers to process`);
    ...
    for (const customer of activeCustomers) {
      console.log(`\n🔄 Processing customer: ${customer.customerName} (DB: ${customer.databaseName})`);
...
async function assignLeadsForCustomer(db, customerName) {
  try {
    ...
    console.log(`[${customerName}] 📊 Online users: ${onlineUsers.length}`);

    if (onlineUsers.length === 0) {
      console.log(`[${customerName}] ⚠️ No online users available for assignment`);
      return;
    }
    ...
    console.log(`[${customerName}] 📋 Unassigned leads: ${unassignedLeads.length}`);

    if (unassignedLeads.length === 0) {
      console.log(`[${customerName}] ✅ No unassigned leads - nothing to do`);
      return;
    }
